...
$build_dir = "/home/liujian/WorkSpace/EBM_Hand/grasp_generation/graspitmodified-build/coin";
while ( $build_dir =~ m%[\\/][^\\/]*[^\\/.][\\/]\.\.% ) {
  $build_dir =~ s%[\\/][^\\/]*[^\\/\.][\\/]\.\.%%g;
}
$build_dir =~ s%[\\/]\.$%%g;
$src_dir = "/home/liujian/WorkSpace/EBM_Hand/grasp_generation/graspitmodified-build/coin/../../graspitmodified_lm/Coin-3.1.3";
while ( $src_dir =~ m%[\\/][^\\/]*[^\\/.][\\/]\.\.% ) {
  $src_dir =~ s%[\\/][^\\/]*[^\\/\.][\\/]\.\.%%g;
}
$src_dir =~ s%[\\/]\.$%%;
...
sub LoadVars {
  local $filename = "/home/liujian/WorkSpace/EBM_Hand/grasp_generation/graspitmodified-build/coin/gendsp-vars.txt";
  open(IN, "${filename}");
  while (<IN>) {
    $line = $_;
    chop($line);
...
if ( $ARGV[0] eq "--register-public-header" ) {
  ...
  $sim_ac_relative_src_dir_q = "../../graspitmodified_lm/Coin-3.1.3";
